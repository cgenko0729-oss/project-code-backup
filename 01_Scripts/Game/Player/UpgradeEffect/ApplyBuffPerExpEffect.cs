...
public class ApplyBuffPerExpEffect : UpgradeEffectBase
{
    [SerializeField, Header("一度に追加される効果量")]
    [Tooltip("サイズ増加量")] public float increaseSizeAmount = 0;
    [Tooltip("クリティカル率増加量")] public float increaseCritAmount = 0;
    ...
    public override void EffectUpdate()
    {
        ...
        for (int i = 0; i < stuckItemDurations.Count; i++)
        {
            ...
            {
                BuffManager.Instance.gobalSizeAdd -= increaseSizeAmount;
                BuffManager.Instance.gobalCritChanceAdd -= increaseCritAmount;
                stuckItemDurations.RemoveAt(i);
                ActiveBuffManager.Instance.ReduceStack(TraitType.DogSkill2_AttackMind);
            }
        }
    }
    ...
    {
        isEnable = BuffManager.Instance.isApplyBuffPerExpEnabled;
        
        if (isEnable == true)
        {
            ActiveBuffManager.Instance.SetStacks(TraitType.DogSkill2_AttackMind, 0);
        }
    }
    ...
    public override bool ActiveBuff()
    {
        ...
        if (stuckItemDurations.Count < activeNumMax)
        {
            ...
            ActiveBuffManager.Instance.AddStack(TraitType.DogSkill2_AttackMind);
            return true;
        }
        return false;
    }
